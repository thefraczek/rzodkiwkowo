      ...
      <nav className="fixed bottom-0 left-0 right-0 z-30 md:hidden bg-white border-t border-gray-200"
        ...
        <div className="flex h-16">
          ...
          <button
            ...
            className={`relative flex-1 flex flex-col items-center justify-center gap-0.5 transition-colors ${
              moreActive ? 'text-green-600' : 'text-gray-400'
            }`}
          >
            {moreActive && (
              <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-green-600 rounded-full" />
          ...
          >
            ...
            <div className="px-4 pb-4 space-y-4">
                  ...
                  {moreOperacyjne.map(item => (
                    <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-xl transition-colors ${
                        pathname === item.href ? 'bg-green-50 text-green-700' : 'text-gray-600 active:bg-gray-100'
                      }`}
                    >
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {moreKonfiguracja.map(item => (
                    <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-xl transition-colors ${
                        pathname === item.href ? 'bg-green-50 text-green-700' : 'text-gray-500 active:bg-gray-100'
                      }`}
                    >
                      <span className="text-2xl">{item.icon}</span>
                      <span className="text-xs font-medium">{item.label}</span>
                ...
                <div className="flex gap-2">
                  {([
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 rounded-xl font-semibold border-2 transition-colors ${opt.cls} ${
                        size === opt.value
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-red-500 active:bg-red-50 transition-colors"
              >
